...
The completed [48-trial automation and agent comparison](AGENT_VALUE_RESULTS.md) restored deterministic and no-agent baselines. Its agent advantage came entirely from a missing observer-failure fallback. The separately declared [80-trial follow-up](RUNBOOK_FALLBACK_RESULTS.md) implemented that fallback: original runbook 14/16, fallback 15/16 with one unknown, basic 15/16, structured 16/16, plus 16 controls. The fallback completed both healthy observer-outage cases and all six regressions without model tokens. Its final trial was interrupted during a host service restart, so its promotion gate remains unmet. The subsequent [service-recovery correction](SERVICE_RECOVERY_RESULTS.md) passed three real whole-service termination gates. The separately declared 32-trial model-free validation passed: 16/16 supported fallback outcomes, 16 unscored controls, complete scoped audits and automatic cleanup. It validates the fallback for this bounded policy without filling the old unknown attempt.
...
| [Service recovery](SERVICE_RECOVERY_RESULTS.md) | Three termination gates passed; separate validation finalized 32/32, with fallback 16/16 and 16 controls | Supervised cleanup and truthful interruption accounting; replay refused; fallback validated for the bounded policy |
| [Mixed-state recovery](MIXED_STATE_RECOVERY_RESULTS.md) | Stop, restart and whole-group SIGKILL each retained one committed result, one unassessed attempt and two unrun trials | Real termination preserves completed evidence and subsequent uncertainty; no persistent-workload claim |
| [Persistent workload](PERSISTENT_CAMPAIGN_RESULTS.md) | A healthy 120-second operator-restart gate passed on GCP and in CI; each retained 12/12 successful scheduled slots and 11 unchanged workload UIDs | Workload and observer outlive the operator; this healthy gate does not exercise repair |
| [Recurring incidents](RECURRENCE_RESULTS.md) | A 300-second campaign passed on GCP and in CI: two repairs across restarts, then budget rejection and escalation; 30 assessed slots, 15 healthy and 15 failed | Authority stays cumulative while measured service failure remains visible; no learning or sustained reliability claim |
...
7. Detaching a cleanup process does not separate it from a systemd control group. The interrupted 80-trial study required offline accounting and manual cleanup. The correction uses a systemd post-stop phase and durable launch claim; three real whole-service gates now pass. An interrupted attempt still remains unknown, and a lifecycle gate is not a completed agent trial.
...
The CLI lab implements real application faults, typed conditional repair, broker budgets and durable intent, independent verification, scoped audit correlation, isolated Docker agents, detached cleanup and a managed cloud service wrapper. Three real gates show its supervised post-stop phase cleans owned resources after the original service processes terminate, preserving interrupted/unrun accounting and refusing replay. Host/power loss or failure of the post-stop phase itself remains outside the tested guarantee. The latest [recurrence gate](RECURRENCE_RESULTS.md) used staged source `c245d39`, with the GCP host confirmed stopped afterward; it did not update the installed v0.1.1 release. The newer managed entry point and campaign code are not retroactively present in that tag. The persistent campaign currently uses trusted deterministic host code; the earlier isolated model-agent trials do not establish isolation for a future continuous model operator. The patched AX variant has separate local recovery evidence; AX has not been validated on GCP.
...
- Remaining continuous-operation gates, versioned procedure promotion/withdrawal and a sustained comparison of service impact, actual human work and full cost. The [goal-alignment review](GOAL_ALIGNMENT.md) fixes this sequence and its evidence requirements.
