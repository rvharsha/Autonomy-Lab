...
The CLI lab implements real application faults, typed conditional repair, broker budgets and durable intent, independent verification, scoped audit correlation, isolated Docker agents, detached cleanup and a managed cloud service wrapper. Three real gates show its supervised post-stop phase cleans owned resources after the original service processes terminate, preserving interrupted/unrun accounting and refusing replay. Host/power loss or failure of the post-stop phase itself remains outside the tested guarantee. The GCP host is stopped after successful validation from staged source `2c2e04e`; its installed v0.1.1 release is unchanged. The new managed cloud entry point is available in that tested staged checkout and the repository; it is not retroactively present in the v0.1.1 tag. The patched AX variant has separate local recovery evidence; AX has not been validated on GCP.
