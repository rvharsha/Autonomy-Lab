...
Run `scripts/check_service_stop.py` separately with positional arguments `stop`, `restart` and `kill` as the host administrator. Each creates a fresh gate identity, waits for an acknowledged repair and checks post-stop recovery. Privileged gate receipts are written under `/var/lib/autonomy-lab/service-gates` (root-owned); the script prints the exact path. Job and experiment evidence remain in the checkout artifacts directory. Do not restart an old prepared job to reproduce it: the durable launch claim deliberately refuses replay. New executions need fresh prepared identities, and failed gates must remain recorded.
