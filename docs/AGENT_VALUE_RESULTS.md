...
The next useful experiment is a separately declared comparison against a deterministic verification fallback, with fresh combinations of observation and workload failures that can also expose an incorrect healthy conclusion. That baseline has not been implemented or tested here. Freeze the new conditions and decision rules before execution; do not reinterpret these 48 trials as validation of a future change.
